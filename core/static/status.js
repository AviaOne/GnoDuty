...
function updateTable(status) {
    ...
    for (let i = 0; i < status.Status.length; i++) {
        ...
        r.insertCell(1).innerHTML = `<div>${_.escape(status.Status[i].name)}${_.escape(status.Status[i].name).includes(_.escape(status.Status[i].chain_id)) ? '' : ' (' + _.escape(status.Status[i].chain_id) + ')'}</div>`
